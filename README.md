...
- Tính giá trị thực chi: `valid_spend = Quantity × unit_price`
...
Sau khi thực hiện Feature Selection, số lượng feature còn lại là 23 feature đủ để train model.
# 3.7 Train model
...
- Nếu kết quả sau khi train khá tốt thì tiến hành hyperparameter tuning với các kỹ thuật:
  - HalvingGridSearchCV: Phase 1, tìm kiếm rộng nhanh
  - GridSearchCV: Phase 2, tinh chỉnh chi tiết xung quanh best params
  - PredefinedSplit: Dùng tập validation riêng thay vì cross-validation
  - Multi-metric Scoring: Theo dõi PR-AUC, Precision, Recall, F1
  - Early Stopping: XGBoost callback, dừng sau 20 rounds không cải thiện
...
| Aspect                             | Chi tiết                                                                                                                                        |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| **Recall cao (83%) ⭐**             | Bắt được gần **5/6 khách sẽ mua lại** → Giảm thiểu miss khách tiềm năng, phù hợp bài toán CRM                                                   |
| **PR-AUC tốt (0.73)**              | Hiệu quả tốt trên tập test cho bài toán **imbalance** → Model phân biệt tốt giữa buyer vs non-buyer                                             |
| **Feature Engineering chất lượng** | Sử dụng **RFM theo time windows (L1M, L3M, L5M)** + **interaction features** |
| **Validation đúng cách ⭐**         | **Time-based split (80/20 theo snapshot_date)** → Tránh data leakage, phản ánh đúng production                                                  |
| **Tuning toàn diện**               | `RandomizedSearchCV` (100 iterations, 5-fold CV, multiple metrics) → Tối ưu model có hệ thống                                                   |
| **Xử lý đa cộng tuyến**            | Loại bỏ feature tương quan cao (>0.9) như `avg_L5M_skus`, `global_cancel_val_ratio` → Giảm overfit, tăng stability                              |
| **Threshold optimization ⭐**       | Chọn ngưỡng **0.4 tối đa F1 (0.6781)** → Có phân tích trade-off Precision vs Recall                                                             |
| **Business-aligned 🎯**            | Phân hạng **3-tier (Hot / Warm / Cold)** → Gắn trực tiếp với chiến lược marketing                                                               |
...
| Vấn đề                              | Mô tả                                                                                                                            |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| **Precision thấp (59%) ⚠️**         | ~41% khách được dự đoán “sẽ mua” là sai → gây **lãng phí ngân sách marketing (~728 khách)**, trade-off của việc tối ưu Recall    |
| **Threshold chọn thủ công ⚠️**      | Chọn ngưỡng 0.4 theo F1 → **chưa tối ưu theo cost thực tế (ROI, CAC, LTV)**                                                      |
| **Chưa có Probability Calibration** | Output probability chưa được calibrate → **không phản ánh đúng xác suất thực tế**, khó dùng cho decision theo ngưỡng             |
| **Model đơn lẻ**                    | Chỉ dùng XGBoost → chưa tận dụng **ensemble (LightGBM, Random Forest)** để tăng stability & generalization                       |
| **Feature selection đơn giản**      | Chỉ loại theo correlation → chưa dùng **RFE / Boruta / SHAP-based selection** → có thể giữ feature không cần thiết               |
...
| Đề xuất                            | Mô tả                                                                                                             |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| **Tối ưu ngưỡng theo lợi nhuận ⭐** | Thay vì tối ưu F1, xây dựng hàm **Profit = TP × Value − FP × Cost** → chọn threshold tối đa hóa **ROI thực tế**   |
| **Ensemble nhiều mô hình**         | Kết hợp **XGBoost + LightGBM + Random Forest** (blending/stacking) → tăng **stability** và khả năng tổng quát hóa |
| **SMOTE + Tomek Links**            | Kết hợp over-sampling + clean boundary → cải thiện **data quality cho class thiểu số**, giảm noise                |
| **Calibrate xác suất ⭐**           | Dùng **Platt scaling / Isotonic regression** → xác suất output phản ánh đúng likelihood → dùng tốt cho decision   |
| **Feature selection nâng cao**     | Áp dụng **SHAP / RFE** → giữ feature quan trọng, giảm overfit, tăng interpretability                              |
| **Bổ sung features mới 🚀**        | Thêm: **lifecycle stage, seasonal pattern, cohort-based features** → giúp model hiểu hành vi dài hạn              |
...
## 4. Link sản phẩm
1. Link github:
- Mock Project: https://github.com/Silas1502/Predict_Repurchase

2. Link production:
- Vercel: https://predict-repurchase-fe-vercel.vercel.app/
- Render: https://predict-repurchase-be.onrender.com